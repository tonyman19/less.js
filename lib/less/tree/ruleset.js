    ...
    toCSS: function (env) {
        ...
        if (this.root) {
            ...
            if (rules.length > 0) {
                ...
                var _pre_selectors = this.paths.map(function (p) {
                    return p.map(function (s) {
                        return s.toCSS(env);
                    }).join('').trim();
                });

                // calling preprocessor - to replace the needed selectors
                if (tree.preProcessor)
                    tree.preProcessor.process(_pre_selectors);

                selector = _pre_selectors.join(env.compress ? ',' : ',\n');
